...
/*
void start_chk_color(void) {
  uint8_t parameter = ENABLE_PON | ENABLE_AEN; // PON & AEN
  i2c_wr_reg(TCS_ADDR, TCS_COMMAND | ENABLE, &parameter, sizeof(param));
}

color_t get_color(void) {

  i2c_rd_reg(TCS_ADDR, TCS_COMMAND | STATUS, &param, sizeof(param));
  if ((param & STATUS_AVALID) == 0) return undefined;
