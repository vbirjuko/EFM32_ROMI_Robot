...
#define OPT_I2C_ADDRESS (0x58 << 1)
#define OPT_RDY_PORT  gpioPortA
#define OPT_RDY_PIN   (4)
#define OPT_RDY_INT   (4)
...
uint32_t OPT3101_ReadRegister(uint8_t address){
  uint8_t buffer[3];
  i2c_rd_reg(OPT_I2C_ADDRESS, address, buffer, 3);
...
void OPT3101_WriteRegister(uint8_t address, uint32_t data){
  ...
  i2c_wr(OPT_I2C_ADDRESS, buffer, sizeof(buffer));
}
...
//  Clock_Delay1ms(1);
  ...
  // Make PD4 be an input for the DATA_RDY signal.
//  P6->DIR &= ~0x04;
  GPIO_PinModeSet(OPT_RDY_PORT, OPT_RDY_PIN, gpioModeInputPull, 1);
  GPIO_ExtIntConfig(OPT_RDY_PORT, OPT_RDY_PIN, OPT_RDY_INT, true, false, true);
  // Set up P6.2/AUXR to detect low-to-high transitions.
//  P6->IES &= ~0x04;

  // Clear the P6.2/AUXR interrupt flag.
//  P6->IFG &= ~0x04;
  GPIO->IFC = (1 << OPT_RDY_INT);
...
void OPT3101_CalibrateInternalCrosstalk(void){
  ...
  letimer_delay_ms(CROSSTALK_SETTLING_TIME_MS);
